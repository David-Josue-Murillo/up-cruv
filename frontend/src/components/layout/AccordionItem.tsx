...
export default function AccordionItem({ item, onNavigate }: Props) {
        ...
        <div className='overflow-hidden'>
          {item.columns.map((col, colIdx) => (
            <div key={colIdx} className='mb-2 last:mb-0'>
              {col.heading && (
                <p className='mb-1.5 pl-3 text-[0.625rem] font-semibold uppercase tracking-widest text-white/50'>{col.heading}</p>
              )}
              {col.links.map((link) => (
                <a
                  key={link.href}
