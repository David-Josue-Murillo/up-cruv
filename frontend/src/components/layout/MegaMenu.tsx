...
export default function MegaMenu({ currentPath = "/" }: { currentPath?: string; transparent?: boolean }) {
  ...
  // Combined keydown + outside-click listener (single effect)
  useEffect(() => {
...
const NavItemButton = memo(function NavItemButton({
  ...
  return (
    <li
      className="relative"
      onMouseEnter={() => hasDropdown && onOpen(index)}
    ...
    >
      ...
      {hasDropdown ? (
        ...
        >
          <div
            className="relative min-w-md overflow-hidden rounded-xl"
